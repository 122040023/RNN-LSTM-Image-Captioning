...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dstep, cache[t])
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = F.cross_entropy(x.reshape(-1, x.shape[2]), y.reshape(-1), ignore_index=ignore_index, reduction='mean')
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type in {"rnn", "lstm"}:
            feats = enc.mean(dim=(2, 3))
            h0 = self.feature_projection(feats)
            A = None
        else:
            conv = enc
            A = self.attn_proj(conv)
            h0 = None

        x_embed = self.embed(captions_in)
        ...
        scores = self.output_projection(h.reshape(N * T, H))
        scores = scores.reshape(N, T, V)

        null_idx = self._null if self.ignore_index is None else self.ignore_index
        loss = temporal_softmax_loss(scores, captions_out, ignore_index=null_idx)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type in {"rnn", "lstm"}:
            feats = enc.mean(dim=(2, 3))
            h = self.feature_projection(feats)
            ...
            conv = enc
            A = self.attn_proj(conv)
            H_feat, W_feat = A.shape[2], A.shape[3]

            h = A.mean(dim=(2, 3))  # (N, H)
            c = h.clone()  # (N, H)

            attn_weights_all = torch.zeros(
                N, max_length, H_feat, W_feat,
                device=device,
                dtype=A.dtype,
            )
        ...
        for t in range(max_length):
            word_vec = self.embed(prev_words.unsqueeze(1)).squeeze(1)
            ...
            if self.cell_type == "rnn":
                ...
                attn_vec, attn_weights = dot_product_attention(h, A)

                h, c = self.rnn.step_forward(word_vec, h, c, attn_vec)
        ...
        if self.cell_type == "attn":
            return captions, attn_weights_all.detach().cpu()
        else:
            return captions
...
def dot_product_attention(prev_h, A):
    ...
    prev_h_exp = prev_h.unsqueeze(2)

    scores = torch.bmm(A_flat.transpose(1, 2), prev_h_exp).squeeze(2)
